...
public class PekerjaanHelper {
    ...
    public ArrayList<Pekerjaan> getPekerjaan(int id){
        ...
        Cursor cursor = db.query(TABLE_PEKERJAAN, null, PEKERJAAN_ID+"='"+id+"'", null, null, null, null, null);
        cursor.moveToFirst();
        Pekerjaan pekerjaan;
        if (cursor.getCount()>0){
            do{
                ...
                pekerjaan.setPekerjaan_fileurl(cursor.getString(cursor.getColumnIndexOrThrow(PEKERJAAN_FILEURL)));
    ...
    private long insert(Pekerjaan pekerjaan) {
        ...
        contentValue.put(PEKERJAAN_FILEURL,pekerjaan.getPekerjaan_fileurl());
